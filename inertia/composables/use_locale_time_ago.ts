...
export function useLocaleTimeAgo(dateOrString: MaybeRefOrGetter<Date | string>) {
  const { t } = useI18n()

  const date = computed(() => {
    return new Date(toValue(dateOrString))
  })
  ...
  return useTimeAgo(date, {
    fullDateFormatter: (d: Date) => d.toLocaleDateString(),
